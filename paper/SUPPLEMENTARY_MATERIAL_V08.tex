...
statement $f(6)=48$.  The manuscript supplies the complete human upper-bound
argument and cites the published lower-bound construction.
...
\end{verbatim}
in namespace \texttt{F6SmallBlockTokenCapacity}.  They prove the
participation bound $|B|-1$, the capacities $q\le1$ for $|B|\le2$ and
...
intersect.  The hypotheses \texttt{HCAP2} and \texttt{HCAP3} explicitly
assert the capacities for convex subsets using at most two and three
participants.  From these hypotheses, the declaration
\begin{verbatim}
F6RotationSupport.widthLE3_and_ci2_of_rotation_support
\end{verbatim}
proves width at most three and CI2.  These capacity hypotheses are visible
arguments, not hidden axioms.  The source package does not construct actual
...
\end{quote}
For the public repository, the audited source files are exposed
...
checks the migrated source against the frozen v07 source.  The support module
may emit unused-section-variable linter warnings; these are not proof
admissions.
The reported load-bearing axioms are \texttt{propext},
\texttt{Classical.choice}, and \texttt{Quot.sound}.

\section*{S6. Scope of the human statements}
The restricted-instance observation, the explicit extremal fifteen-element
poset, the necessary equality conditions, and the $DS_3$--$P_*$ isomorphism
are human statements, not Lean-verified statements in the bundled formalization.
The general order-$N$ support-budget statement and the stable-matching--downset
correspondence also remain outside the bundled formal development.
These statements are not formalized in the accompanying Lean development.
The upper bound does not rely
on the auxiliary example calculations below.
...
antichains, and maximum common-incomparability size two.  This verifies one
explicit extremal example and does not classify extremal posets.
...
These are checks of one instance, not a proof of the universal upper bound
or of the necessity statements for every extremal instance.
...
this join-irreducible poset is isomorphic to the rotation poset of $DS_3$.  This is a finite certificate for one extremal example.
It is not used in the universal upper bound and does not prove uniqueness
of the extremal rotation poset.  No novelty or priority claim is made for
this isomorphism.
...
\texttt{SHA256SUMS.txt}.  These auxiliary checks concern one explicit
example and are not a universal proof or Lean verification.
