...
Equality forces fifteen rotations, all of size two, and makes all $36$
man--woman pairs attainable.  Accompanying Lean files verify the fixed-token, abstract
support, and finite-poset components; the connection to classical rotation
semantics is given in the human proof.
\end{abstract}
...
by a local restriction on rotation posets and a sharp abstract poset theorem.
...
using an entropy argument.  We work within this established framework.
In particular, the downset correspondence, shared-agent comparability, and
the global rotation bound are not new ingredients of the present argument.

Our local input concerns an order-convex block $Q$ of rotations.  If the
...
exhaustive classification of preference profiles or saturated rotation
instances.  The consecutiveness property of an already ordered family of
...
A 2026 departmental project announcement at Biola University reports the
same numerical result; at the time of writing, we were unable to locate a
public proof, and we make no priority claim.\footnote{Biola University,
Department of Math and Computer Science, ``Current Students: Completed
Projects,'' entry for Roman Parks (2026),
...
distinct current partners.  An arbitrary list of matched pairs is not a
rotation.  Elimination replaces $(m_i,w_i)$ by $(m_i,w_{i+1})$, with indices
...
the three supports.  Thus, naming $x$ as the man common to the first and third supports
and $a,b$ as the other two men, the supports in temporal order are
\[
...
\begin{remark}[Restriction to the participating agents]\label{rem:restriction}
There is a complementary interpretation of a nonempty convex block $Q$.
Let $J=(\down Q)\setminus Q$, let $B=B(Q)$, and let $W$ be the partners
of $B$ in the matching corresponding to $J$.  For every downset $K$ of the
induced poset $Q$, the set $J\cup K$ is a downset of the full rotation
poset.  Indeed, if $y\in J$ and $x\le y$, then $x\in J$ because
$J$ is a downset.  If $y\in K\subseteq Q$ and $x\le y$, then
$x\in\down Q=J\cup Q$.  If $x\in Q$, the induced-downset property
of $K$ gives $x\in K$; otherwise $x\in J$.
Its stable matching restricts to a stable matching of the instance
obtained by restricting preferences to $B$ and $W$: any blocking pair in
the restriction would also block the global matching.  If $K_1\ne K_2$, then $J\cup K_1\ne J\cup K_2$, so the
stable-matching/downset bijection gives distinct global stable matchings.
During the $Q$-window all matching changes occur inside $B\times W$;
outside $B\times W$ the matching is fixed.  Hence the two global matchings
can differ only through their restrictions to $B\times W$, and those
restrictions are distinct.
Writing $\mathcal J(Q)$ for the downsets of $Q$, we obtain
\[
        |Q|+1\le |\mathcal J(Q)|\le f(|B|).
\]
The first inequality follows from the prefixes of a linear extension of
$Q$.  Using the elementary exact values $f(2)=2$ and $f(3)=3$, this also
recovers the two numerical small-block capacity conclusions.  We retain the
direct token proof because it supplies the participation/incidence bound
used later and does not use those external small-order values.
\end{remark}
...
induced diamond and no induced $K_{2,3}$.  Thus the pure order-theoretic
part of the proof concerns a restricted class of cocomparability graphs.
Antichains of the poset are precisely cliques of its incomparability graph.
\end{remark}
...
\[
        ...
        \quad t=A_3.
\]
The following no-re-entry property is the standard consecutiveness
...
$U$.  We separate two localization steps.  First localize the possible new
cross edges at a transition
$T_i\to T_{i+1}$.  If
\[
...
Analogously, $y\in T_{i+1}\setminus U_i$ cannot lie below a member
...
For the second localization step, let $x\notin U$.  For every triple
...
Lemma~\ref{lem:triple} gives at most one such neighbor in each triple, so
\begin{equation}\label{eq:outsider-neighbor-bound}
|\Inc(x)\cap U|\le2.
\end{equation}
If all triples lie on one side of $x$, the same argument gives the stronger
bound one.
...
family of three-element antichains to couple the two counts; no
classification of fifteen-element posets is involved.
\end{remark}
...
Thus all $36$ man--woman pairs are attainable.  The conclusion is a set of
necessary conditions, not a uniqueness or sufficiency assertion.
\end{proof}
...
not used in the upper-bound proof and is not asserted as a
uniqueness/classification statement.
\end{remark}
...
stable-matching--downset correspondence remain human mathematics supported
by the classical references above.
The Supplementary Material records the precise formalization scope and
reproducibility details.

\section*{Research provenance and responsibility}

Generative-AI systems from OpenAI were used substantively during the
research workflow, including exploration of proof strategies and auxiliary
lemmas, assistance with finite-verification and Lean-formalization workflows,
and drafting or revising parts of the exposition.  The human author selected
the research objectives and final proof route, checked the mathematical
arguments and computational certificates, reviewed the formalization
boundaries, and takes full responsibility for every statement in the
manuscript.  No AI system is an author.  The accompanying repository records
verification artifacts and the scope of the formalized components.  The
retained project records do not support a complete exact model/version
history, so no such history is asserted here.
